...
class _PerformanceSampler(threading.Thread):
    ...
    def run(self):
        while not self._stop_event.is_set():
            ...
            total_ram_mb = self.service.get_total_ram_usage_mb()
            ...
            self.service._record_performance_sample(timestamp, total_ram_mb, cpu_percent_system)
...
class BenchmarkService:
    ...
    def _record_performance_sample(self, timestamp: float, total_ram_mb: float, cpu_percent_system: float | None = None) -> None:
        sample = {
    ...
    def set_phase(self, phase: str) -> None:
        ...
        self._record_performance_sample(timestamp, total_ram_mb, cpu_percent_system)
    ...
    def log_results_to_csv(self):
        ...
        curves_header = "timestamp_s,phase,total_ram_mb,total_used_ram_percent,cpu_percent_system\n"
        with open(curve_file, "w") as curves_file:
            ...
            for sample in self._performance_curve:
                ...
                total_used_ram_percent = (f"{(total_ram_mb / self._total_ram_mb * 100.0):.2f}" if self._total_ram_mb else "0.00")
                ...
                curves_file.write(f"{timestamp_s:.4f},{phase},{total_ram_mb:.2f},{rounded_total_used_ram_percent},{rounded_cpu_percent_system}\n")
    ...
    def print_summary(self) -> None:
        ...
        if self._performance_curve:
            for sample in self._performance_curve:
                ...
                print(f"Time: {timestamp:.2f} s, Phase: {phase}, Total RAM: {ram_mb_total:.2f} MB {cpu_system_str}")
        else:
